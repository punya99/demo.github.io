...
! function($) {
    ...
    function Carousel(element, options) {
        this.$element = $(element), this.$indicators = this.$element.find(".vc_carousel-indicators"), this.options = options, this.paused = this.sliding = this.interval = this.$active = this.$items = null, "hover" === this.options.pause && this.$element.on("mouseenter", $.proxy(this.pause, this)).on("mouseleave", $.proxy(this.cycle, this)), this._build()
    }
    Carousel.DEFAULTS = {
        mode: "horizontal",
        partial: !1,
        ...
        return e || (this.paused = !1), this.interval && clearInterval(this.interval), this.options.interval && !this.paused && (this.interval = setInterval($.proxy(this.next, this), this.options.interval)), this.touch_start_position = 0, this
        ...
        return e || (this.paused = !0), this.$element.find(".vc_right.vc_carousel-control, .vc_left.vc_carousel-control").length && $.support.transition.end && (this.$element.trigger($.support.transition.end), this.cycle(!0)), this.interval = clearInterval(this.interval), this
        ...
        if (!$next.hasClass("vc_active")) {
            if (this.$indicators.length && (this.$indicators.find(".vc_active").removeClass("vc_active"), this.$indicators.find(".vc_partial").removeClass("vc_partial"), this.$element.one("slid", function() {
                    ...
                    $nextIndicator && $nextIndicator.addClass("vc_active"), that.options.partial && $nextIndicator && (index + 1 < that.items_count ? $nextIndicator.last().next() : $nextIndicator.first().prev()).addClass("vc_partial"), that.options.wrap || that.showHideControl(index)
                })), this.current_index = $next.index(), that.options.wrap || this.showHideControl(this.current_index), this.current_index > this.items_count ? this.current_index = 0 : this.current_index < 0 && (this.current_index = this.items_count - 1), this.options.autoHeight ? this.current_pos_value = -1 * this._step * this.current_index : this.current_pos_value = -1 * $next.position()[this.animation_position], this.options.partial && this.current_index >= this.items_count - 1 && (this.current_pos_value += this._step * (1 - this.partial_part)), $.support.transition && this.$element.hasClass("vc_slide")) {
                if (this.$element.trigger(e), e.isDefaultPrevented()) return;
                this.$slideline_inner.addClass("vc_transition").css(this.animation_position, this.current_pos_value + that.pos_units), this.options.autoHeight || this.recalculateSlidelineHeight($next.height(), !0), this.$slideline_inner.one($.support.transition.end, function() {
                    $next.addClass("vc_active"), $active.removeClass("vc_active"), that.$slideline_inner.removeClass([type, "vc_transition"].join(" ")), that.sliding = !1, that.removeSwipeAnimationSpeed(), setTimeout(function() {
        ...
        !0 === animate ? this.$slideline.animate({
            height: height
        }) : this.$slideline.height(height)
    }, Carousel.prototype.resizeAction = function() {
        var new_slideline_height, max_height = 0;
        ...
        }), this.$items.height(max_height)) : this.recalculateSlidelineHeight(this.$active.height()), "vertical" === this.options.mode && (this._step = this.$active.height(), new_slideline_height = this.$active.height() * this.options.perView * (this.options.partial ? 2 - this.partial_part : 1), this.recalculateSlidelineHeight(new_slideline_height, !1), this.$slideline_inner.css({
        ...
        }.bind(this), !1), el.addEventListener("touchmove", function(e) {
            return _diff = parseFloat(e[that.touch_direction]) - _touch_start_position, !(_moved = 0 < Math.abs(_diff)) || (e.preventDefault(), void(that.slideline_inner.style[that.animation_position] = _pos_before_touch + _diff + "px"))
        }, !1), el.addEventListener("touchend", function(part) {
            var velocity;
            _moved && (velocity = (part.timeStamp - _touch_start_time) / 1e3, part = _diff / that.el_effect_size, (velocity = that.velocity(velocity, part)).x > that.swipe_velocity && part < 0 || part <= -.7 ? (that.setSwipeAnimationSpeed(), that.next()) : velocity.x > that.swipe_velocity || .7 <= part ? (that.setSwipeAnimationSpeed(), that.prev()) : that.returnSwipedSlide(), _moved = !1)
